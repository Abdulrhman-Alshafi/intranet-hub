...
const IconHeart: React.FC<{ filled: boolean }> = ({ filled }) => (
  <svg width="14" height="14" viewBox="0 0 24 24" fill={filled ? '#ef4444' : 'none'} stroke={filled ? '#ef4444' : 'currentColor'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
...
const KudosCard: React.FC<IKudosCardProps> = ({ item, isAdmin, siteUrl, onDelete, onHide }) => {
  ...
  return (
    <div className={`${styles.kudosCard} ${item.IsHidden ? styles.hiddenCard : ''}`}>
      <div className={styles.kudosAvatar}>
        {photoUrl
          ? <img src={photoUrl} alt={item.Recipient?.Title || ''} onError={() => setImgError(true)} />
        ...
        {isAdmin && (
          <div className={styles.adminActions}>
            <button
              ...
              onClick={() => onHide?.(item.Id, !item.IsHidden)}
            >
